...
class PGTAOICRROIHeads(ROIHeads):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # pgta
        self.cls_fc = nn.Conv2d(512, 20, 1, bias=False)
        torch.nn.init.xavier_uniform_(self.cls_fc.weight)
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        pooler_scales     = tuple([1.0 / input_shape["plain5"].stride])
        ...
        box_head = build_box_head(
            cfg, ShapeSpec(channels=in_channels[-1], height=pooler_resolution, width=pooler_resolution)   # pgta only use the last in_channels
        )
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.visualize_pgta:
                pred_instances, all_scores, all_boxes, pgta_data = self._forward_box(features, proposals)
            else:
    ...
    def _forward_box(
        ...
        # generate backward pgt
        GEN_BACKWARD_PGT = False
        if GEN_BACKWARD_PGT:
