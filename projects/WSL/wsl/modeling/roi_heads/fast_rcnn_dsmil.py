...
class DSMILOutputLayers(nn.Module):
    ...
    def forward(self, x, proposals=None, context=False):
        ...
        # ds attention
        Q = self.q(x)    # N * Q
        _, m_indices = torch.sort(scores, 0, descending=True)
        center_x = torch.index_select(x, dim=0, index=m_indices[0, :])   # C * D
        Q_max = self.q(center_x)   # C * Q
        attentions = torch.mm(Q, Q_max.transpose(0, 1))   # N * C
        attentions = F.softmax(attentions / torch.sqrt(torch.tensor(Q.shape[1], dtype=torch.float32, device=Q.device)), 0)   # N * C
